...
columnDefs.value = [
{
        ...
        cellRenderer: (params: { data:IToolEvaluation}) => {
            ...
            span.innerText = params.data.name; // Set the text
...
const viewRecord = (facility: string, completionLevel: string) => {
    //router.visit('/items/' + record + '/edit', { method: 'get'});
  navigateTo(`/ReportPlatform/Reporting/facilities/${facility}/${completionLevel}`)
};
